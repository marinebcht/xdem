...
class TestProfiling:
    ...
    def test_profiling_configuration(self, profiling_configuration, profiling_function, tmp_path) -> None:
        ...
        if s_rd or s_gr:
            assert op.isdir(op.join(tmp_path, "profiling"))

            if s_rd:
                assert op.isfile(op.join(tmp_path, "profiling", "raw_data.pickle"))
                df = pd.read_pickle(op.join(tmp_path, "profiling", "raw_data.pickle"))
                if profiling_function == "coreg":
                    assert len(df) == 3
                ...
                assert not op.isfile(op.join(tmp_path, "profiling", "raw_data.pickle"))

            if s_gr:
                assert op.isfile(op.join(tmp_path, "profiling", "time_graph.html"))
                assert op.isfile(op.join(tmp_path, "profiling", "memory_dem.__init__.html"))
                if profiling_function == "coreg":
                    assert op.isfile(op.join(tmp_path, "profiling", "memory_coreg.base.fit_and_apply.html"))
                if profiling_function == "attributes":
                    assert op.isfile(op.join(tmp_path, "profiling", "memory_terrain.slope.html"))
                    assert op.isfile(op.join(tmp_path, "profiling", "memory_terrain.get_terrain_attribute.html"))
                if profiling_function == "attribute_by_get_terrain_attribute":
                    assert op.isfile(op.join(tmp_path, "profiling", "memory_terrain.get_terrain_attribute.html"))

            else:
                assert not len(glob.glob(op.join(tmp_path, "profiling", "*.html")))
        else:
            assert not op.isdir(op.join(tmp_path, "profiling"))
